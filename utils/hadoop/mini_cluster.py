...
from utils.lib import python_util

from utils.hadoop.fs.hadoopfs import HadoopFileSystem
from utils.hadoop.job_tracker import LiveJobTracker
import utils.hadoop.cluster as hadoop_cluster
...
""")
    ...
    try:
      args = [ os.path.join(hadoop.conf.HADOOP_MR1_HOME.get(), 'bin', 'hadoop'),
        "jar",
        hadoop_conf.HADOOP_TEST_JAR.get(),
        "minicluster",
      ...
      env["HADOOP_CLASSPATH"] = ':'.join([
        ...
        hadoop_conf.HADOOP_TEST_JAR.get(),
        # -- END JAVA TRIVIA --
        hadoop_conf.HADOOP_PLUGIN_CLASSPATH.get(),
      ...
      env["HADOOP_HOME"] = hadoop_conf.HADOOP_MR1_HOME.get()
    ...
    hadoop_conf.HADOOP_CONF_DIR.set_for_testing(self.config_dir)
    ...
    args = [hadoop_conf.HADOOP_BIN.get(),
  ...
  def fs(self):
    ...
    if self._fs is None:
      self._fs = HadoopFileSystem("localhost", 
        ...
        hadoop_bin_path=hadoop_conf.HADOOP_BIN.get())
    return self._fs
...
def shared_cluster(conf=False):
  ...
  if conf:
    closers.extend([
      hadoop_conf.HDFS_CLUSTERS["default"].NN_HOST.set_for_testing("localhost"),
      hadoop_conf.HDFS_CLUSTERS["default"].NN_HDFS_PORT.set_for_testing(cluster.namenode_port),
      hadoop_conf.MR_CLUSTERS["default"].HOST.set_for_testing("localhost"),
      hadoop_conf.MR_CLUSTERS["default"].JT_THRIFT_PORT.set_for_testing(cluster.jt.thrift_port),
    ])
    # Clear the caches
    # This is djanky (that's django for "janky").
    # Caches are tricky w.r.t. to to testing;
    # perhaps there are better patterns?
    old = hadoop_cluster.clear_caches()

  def finish():
    if conf:
      hadoop_cluster.restore_caches(old)
    for x in closers:
      x()
...
if __name__ == '__main__':
  ...
  from utils.lib.conf import initialize
  initialize([hadoop_conf])
