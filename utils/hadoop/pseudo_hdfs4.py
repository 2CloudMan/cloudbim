...
from utils.lib.python_util import find_unused_port

from utils import hadoop
import utils.hadoop.fs
from utils.hadoop.mini_cluster import write_config
from utils.hadoop.job_tracker import LiveJobTracker
from utils.lib.paths import get_run_root
  ...
  def fs(self):
    if self._fs is None:
      if self._dfs_http_address is None:
        ...
        return None
      self._fs = utils.hadoop.fs.webhdfs.WebHdfs("http://%s/webhdfs/v1" % (self._dfs_http_address,), self.fs_default_name)
    return self._fs
