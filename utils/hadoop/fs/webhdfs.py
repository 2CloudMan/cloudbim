...
from utils.lib.rest import http_client, resource
...
from utils.hadoop.fs.webhdfs_types import WebHdfsStat, WebHdfsContentSummary
from utils.hadoop.conf import UPLOAD_CHUNK_SIZE
from utils.hadoop.hdfs_site import get_nn_sentry_prefixes, get_umask_mode

import utils.hadoop.conf
import utils.conf

DEFAULT_HDFS_SUPERUSER = utils.conf.DEFAULT_HDFS_SUPERUSER.get()
...
class WebHdfs(Hdfs):
  ...
  """
  DEFAULT_USER = utils.conf.DEFAULT_USER.get()        # This should be the user running Hue
...
def _get_service_url(hdfs_config):
  ...
  host = netloc.split(':')[0]
  port = utils.hadoop.conf.DEFAULT_NN_HTTP_PORT
