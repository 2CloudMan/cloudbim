...
import utils.conf
import utils.lib.thrift_util
from utils.lib import django_mako
from utils.lib.json_utils import JSONEncoderForHTML
...
class Encoder(json.JSONEncoder):
  ...
  def default(self, o):
    ...
    if utils.lib.thrift_util.is_thrift_struct(o):
      return utils.lib.thrift_util.thrift2json(o)
...
def get_desktop_uri_prefix():
  ...
  if _uri_prefix is None:
    dt_host = utils.conf.HTTP_HOST.get()
    if dt_host == '0.0.0.0':
      dt_host = socket.getfqdn()
    protocol = utils.conf.is_https_enabled() and "https" or "http"
    _uri_prefix = '%s://%s:%s' % (protocol, dt_host, utils.conf.HTTP_PORT.get())
  return _uri_prefix
