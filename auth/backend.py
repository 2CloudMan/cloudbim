...
def load_augmentation_class():
  ...
  """
  try:
    class_name = utils.conf.AUTH.USER_AUGMENTOR.get()
    i = class_name.rfind('.')
...
class PamBackend(DesktopBackendBase):
  ...
  """
  def check_auth(self, username, password):
    if pam.authenticate(username, password, utils.conf.AUTH.PAM_SERVICE.get()):
      is_super = False
...
class LdapBackend(object):
  ...
  def __init__(self):
    ...
    class _LDAPBackend(LDAPBackend):
      def get_or_create_user(self, username, ldap_user):
        username = utils.conf.LDAP.FORCE_USERNAME_LOWERCASE.get() and username.lower() or username
        if utils.conf.LDAP.IGNORE_USERNAME_CASE.get():
          try:
  ...
  def add_ldap_config_for_server(self, server):
    if utils.conf.LDAP.LDAP_SERVERS.get():
      # Choose from multiple server configs
      if server in utils.conf.LDAP.LDAP_SERVERS.get():
        self.add_ldap_config(utils.conf.LDAP.LDAP_SERVERS.get()[server])
    else:
      self.add_ldap_config(utils.conf.LDAP)
  ...
  def authenticate(self, username=None, password=None, server=None):
    ...
    elif not utils.conf.LDAP.CREATE_USERS_ON_LOGIN.get():
      return None
    ...
    if user is not None and user.is_active:
      ...
      if utils.conf.LDAP.SYNC_GROUPS_ON_LOGIN.get():
...
class RemoteUserDjangoBackend(django.contrib.auth.backends.RemoteUserBackend):
  ...
  def authenticate(self, remote_user=None):
    ...
    username = utils.conf.AUTH.FORCE_USERNAME_LOWERCASE.get() and username.lower() or username
    is_super = False
    if User.objects.count() == 0:
      is_super = True

    try:
      if utils.conf.AUTH.IGNORE_USERNAME_CASE.get():
